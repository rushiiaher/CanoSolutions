            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8 md:gap-10">
              {stats.map((stat, index) => (
                <div key={index} className="text-center transform hover:-translate-y-1 transition-transform duration-300">
                  <div className="text-2xl sm:text-3xl md:text-5xl font-bold text-primary mb-3">{stat.number}</div>
      ...
      <section className="py-12 sm:py-16 md:py-20 bg-white">
        <div className="container mx-auto px-4">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-3xl sm:text-4xl font-bold text-textPrimary mb-4">Our Expert Solutions</h2>
              ...
              return (
                ...
                >
                  ...
                  <CardHeader className="p-6 sm:p-8">
                    <div className="w-16 h-16 mx-auto mb-6 rounded-full bg-primary/10 flex items-center justify-center shadow-[0_5px_15px_rgba(30,136,229,0.15)]">
                      <IconComponent className="w-7 h-7 text-primary" />
                    </div>
                    <CardTitle className="text-xl sm:text-2xl text-textPrimary group-hover:text-primary transition-colors text-center">
                      {service.title}
                    </CardTitle>
                    <CardDescription className="text-textSecondary text-sm sm:text-base text-center mt-2">{service.description}</CardDescription>
                  </CardHeader>
                  <CardContent className="p-6 sm:p-8 pt-0">
                    <ul className="space-y-3 mb-8">
                      {service.features.map((feature, idx) => (
                        <li key={idx} className="flex items-center text-textSecondary text-sm sm:text-base">
                          <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 text-secondary mr-3 flex-shrink-0" />
                          {feature}
                        </li>
                      ))}
                    </ul>
                    <Link href={service.href} className="block transform hover:translate-y-[-2px] transition-transform duration-300">
                      <Button
                        variant="outline"
                        className="w-full border-primary text-primary hover:bg-primary hover:text-white bg-transparent touch-target font-medium rounded-lg"
                      >
                        Learn More
